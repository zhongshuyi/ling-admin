...
public class DataPermissionHelper {
     ...
     * 获取用户的数据权限信息.
     *
     * @param adminUserDetails 用户信息.
     * @return 数据权限列表
     */
    public static List<DataScopeTypeEnum> getDataPermissionTypeEnum(final AdminUserDetails adminUserDetails) {
        // 用户角色id列表
        final List<SysRole> sysRoles = adminUserDetails.getRoles();
        // 部门id列表
        final List<Long> deptIds =
                adminUserDetails.getDepts().stream().map(SysDept::getId).collect(Collectors.toList());
        // 获取所有角色的数据权限
        final Set<Integer> dataScopeCodes =
                sysRoles.stream().map(SysRole::getDataScope).collect(Collectors.toSet());
        // 角色与部门都为空则抛出异常
        if (CollUtil.isEmpty(sysRoles) && CollUtil.isEmpty(deptIds)) {
        ...
        // 如果角色为空则只查询本部门权限
        if (CollUtil.isEmpty(sysRoles) && CollUtil.isNotEmpty(deptIds)) {
            return List.of(DataScopeTypeEnum.DEPT);
        }
        // 如果部门为空但有角色则只给查询本人数据
        if (CollUtil.isNotEmpty(sysRoles) && CollUtil.isEmpty(deptIds)) {
            return List.of(DataScopeTypeEnum.ME);
        }
        // 角色与部门都不为空
        if (CollUtil.isNotEmpty(sysRoles) && CollUtil.isNotEmpty(deptIds)) {
            // 如果没有给角色赋数据权限则给本部门权限
    ...
    public Expression getNewWhere(
        ...
        for (final Integer code : adminUserDetails.getDataScopeTypeEnumCodeList()) {
    ...
    private Expression buildNewWhere(
        ...
        final AdminUserDetails adminUserDetails = SecurityUtils.getLoginUser();
        // 用户id
        final Long userId = adminUserDetails.getSysAdmin().getId();
        // 用户角色id列表
        final List<Long> sysRoleIds =
                adminUserDetails.getRoles().stream().map(SysRole::getId).collect(Collectors.toList());
        // 部门id列表
        final List<Long> deptIds =
                adminUserDetails.getDepts().stream().map(SysDept::getId).collect(Collectors.toList());
        try {
