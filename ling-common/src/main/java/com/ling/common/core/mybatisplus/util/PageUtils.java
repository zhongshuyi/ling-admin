...
public final class PageUtils {
     ...
     * 排序列.
     */
    private static final String ORDER_BY_COLUMN = "field";
    /**
     * 前端传递排序方向的键名.
     */
    ...
    public static <T, K> PagePlus<T, K> buildPagePlus() {
        ...
        final String isAsc = ServletUtils.getParameter(ORDER_KEY);
        if (pageNum <= 0) {
    ...
    private static <T> Page<T> buildPage(
        ...
        final String isAsc = ServletUtils.getParameter(ORDER_KEY, defaultIsAsc);
        log.info("排序列:" + orderByColumn + "排序方向:" + isAsc);
        if (pageNum <= 0) {
            pageNum = DEFAULT_PAGE_NUM;
        }
